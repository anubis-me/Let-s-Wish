...
const toArray = function toArray(val) {
  ...
  else if (is.undef(val) || is.nil(val)) {
    return []
  }
  else {
    return Array.of(val)
...
const validate = function validate(options) {
  ...
  if (!is.function(options.validator) && !is.string(options.validator)) {
    throw new Error(`validator must be of type function or string, received ${typeof options.validator}`)
  }
  ...
  const messageStr    = findFirstString(options.message, customErrorMessages[validatorName], defaultErrorMessages[validatorName], 'Error')
